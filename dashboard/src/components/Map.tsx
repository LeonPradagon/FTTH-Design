...
const parseKmlTree = (parentEl: Element, doc: Document): KmlNode[] => {
  ...
  for (let i = 0; i < parentEl.children.length; i++) {
    ...
    if (tagName === 'Folder' || tagName === 'Document' || tagName === 'kml') {
      ...
      const visibilityEl = Array.from(child.children).find(c => c.tagName.replace(/.*:/, '') === 'visibility');
      let visible = true;
      if (visibilityEl && visibilityEl.textContent === '0') visible = false;
...
const findNodeVisible = (nodes: KmlNode[] | undefined, targetId: string): boolean | null => {
  ...
  for (const node of nodes) {
    ...
    if (node.children) {
      ...
      if (res !== null) return node.visible && res;
    }
  }
  return null;
};

const isNodeInFolder = (nodes: KmlNode[] | undefined, targetId: string, keywords: string[], inTargetFolder = false): boolean => {
  if (!nodes) return false;
  for (const node of nodes) {
    const nodeNameUpper = node.name.toUpperCase();
    const matchesKeyword = keywords.some(kw => nodeNameUpper.includes(kw));
    const isTarget = inTargetFolder || matchesKeyword;
    
    if (node.id === targetId) return isTarget;
    if (node.children) {
      if (isNodeInFolder(node.children, targetId, keywords, isTarget)) return true;
    }
  }
  return false;
};
  ...
  const deckLayers = useMemo(() => {
    return layers
      ...
      .map((layer) => {
        ...
        // We track the IMMEDIATE parent folder name to correctly classify items,
        // because KML structure is: "ODC 1" (wrapper) > "ODC"/"ODP"/"HC" (type folders) > placemarks
        const treeInfoMap = new Map<string, {
            visible: boolean | null;
            immediateParent: string;  // name of the direct parent folder (e.g. "ODC", "ODP", "HC")
        }>();

        if (kmlTrees && kmlTrees[layer.id]) {
            const traverseTree = (nodes: KmlNode[], parentVisible: boolean | null, parentFolderName: string) => {
                for (const node of nodes) {
                    const vis = parentVisible === false ? false : node.visible;
                    
                    if (node.type === 'placemark') {
                        treeInfoMap.set(node.id, {
                            visible: vis,
                            immediateParent: parentFolderName
                        });
                    }
                    
                    if (node.children) {
                        // Pass this folder's name as the immediate parent for its children
                        traverseTree(node.children, vis, node.name);
                    }
                }
            };
            
            traverseTree(kmlTrees[layer.id], null, "");
        }
        ...
        const getFeatureInfo = (f: any) => {
            ...
            // Get immediate parent folder from pre-computed tree info
            let parentUpper = "";
            ...
            if (f.properties?.treeId) {
                ...
                if (info) {
                    ...
                    parentUpper = info.immediateParent.toUpperCase();
                }
            }

            // Classify based on immediate parent folder name first, then fallback to feature name/description
            const inPopFolder = parentUpper.includes("OLT") || parentUpper.includes("POP") || parentUpper.includes("SENTRAL");
            const inOdcFolder = parentUpper === "ODC";  // exact match — not "ODC 1" wrapper
            const inClosureFolder = parentUpper.includes("CLOSURE");
            const inOdpFolder = parentUpper === "ODP";  // exact match
            const inHouseFolder = parentUpper === "HC" || parentUpper.includes("HOME PASS") || parentUpper.includes("HOUSE") || parentUpper.includes("RUMAH");

            const isPop = isPoint && (inPopFolder || layer.id === 'pop' || desc.includes("SERVER OLT") || nameUpper.includes("POP") || nameUpper.includes("OLT"));
            const isOdc = isPoint && (inOdcFolder || nameUpper.startsWith("ODC") || desc.includes("Jumlah ODP:"));
            const isClosure = isPoint && (inClosureFolder || nameUpper.includes("JOIN CLOSURE"));
            const isOdp = isPoint && (inOdpFolder || (/^\d{1,2}\/\d{1,2}$/.test(name)) || desc.includes("Induk: ODC") || nameUpper.includes("ODP"));
            const isHouse = isPoint && (inHouseFolder || (/^\d{1,2}\/\d{1,2}-\d{1,2}$/.test(name)) || desc.includes("Induk ODP:") || (!isPop && !isOdc && !isClosure && !isOdp));
            
            const isFeeder = isLine && (nameUpper.includes("FEEDER") || nameUpper.includes("LINE FD") || nameUpper.startsWith("FD ") || desc.includes("FEEDER") || f.properties?.stroke === "#ff0000");
