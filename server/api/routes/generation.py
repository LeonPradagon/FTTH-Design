...
from server.services.generator.osm_local import fetch_houses_in_boundary, fetch_road_graph
...
def cleanup_old_files(directory=DATA_DIR, max_age_seconds=3600):
...
async def generate_design(
    ...
    if boundaryFile and boundaryFile.filename:
        ...
        boundary_path = str(DATA_DIR / f"custom_boundary_{int(time.time())}.kml")
        with open(boundary_path, "wb") as buffer:
            shutil.copyfileobj(boundaryFile.file, buffer)

    if popFile and popFile.filename:
        DATA_DIR.mkdir(parents=True, exist_ok=True)
        pop_path = str(DATA_DIR / f"custom_pop_{int(time.time())}.kml")
    ...
    output_kmz_path = str(DATA_DIR / output_kmz_name)
    output_kml = str(DATA_DIR / f"design_ftth_{timestamp}.kml")
    output_csv_name = f"design_ftth_{timestamp}.csv"
    output_csv_path = str(DATA_DIR / output_csv_name)
...
async def regenerate_cables():
    ...
    output_kmz_path = str(DATA_DIR / output_kmz_name)
    output_kml = str(DATA_DIR / f"design_ftth_regen_{timestamp}.kml")
...
async def generate_custom(
    ...
    timestamp = int(time.time())
    custom_path = str(DATA_DIR / f"custom_mapping_{timestamp}.kml")
